...
# 1. Custom CSS: Bottom-Left, Perfect Circle, and Ultra-Fast Animations
st.markdown("""
    <style>
    /* Float the popover button to bottom LEFT */
    ...
    /* Style the button into a perfect, fast-animating circle */
    div[data-testid="stPopover"] > button {
        ...
        font-size: 28px !important; /* Large chat icon */
        border: 2px solid #c084fc !important;
        box-shadow: 0 8px 16px rgba(0,0,0,0.3) !important;
        transition: all 0.1s cubic-bezier(0.4, 0, 0.2, 1) !important; /* Ultra-fast, snappy transition */
    }
    div[data-testid="stPopover"] > button:hover {
        transform: scale(1.15) translateY(-4px); /* Bigger, faster pop */
...
# 3. Create the Popover (Using just a speech bubble icon to keep the circle shape)
with st.popover("💬"):
    
    # Header inside the popup (Slightly darker purple for better readability)
    st.markdown("<div style='color: #6b21a8; font-family: Orbitron; font-weight: 900; font-size: 16px; border-bottom: 1px solid #e2e8f0; padding-bottom: 10px; margin-bottom: 10px;'>🤖 SemiconBot OS v2.1</div>", unsafe_allow_html=True)
    
    # Chat container with a fixed height
    ...
    if prompt := st.chat_input("Transmit message..."):
        ...
        # ------------------------------------------------
        # Enhanced Conversational & Analytical Logic
        lower_prompt = prompt.lower()
        
        # Generic ChatGPT-style greetings
        if lower_prompt in ["hi", "hello", "hey", "greetings"]:
            ...
            bot_reply = "All systems are nominal. My GIS routing subroutines are running at peak efficiency. What's our next objective?"
        ...
        # Hardcoded strategic data
            ...
            bot_reply = "I am currently running on localized GIS logic. To unlock full Generative AI capabilities, an OpenAI or Gemini API key must be inserted into the source code."
        else:
            bot_reply = f"Scanning databases for: '{prompt}'... \n\n*Topographical matrices processed. I do not have specific data on that parameter yet. Try asking about 'Dholera', 'TSMC', or 'India'.*"
        # ------------------------------------------------
        ...
        # Forces the app to instantly refresh and stay inside the popup
        st.rerun()
