...
# Premium F1 / Cinematic Black & Gold CSS
st.markdown("""
    <style>
        ...
        .news-card-grid { width: 100%; height: 180px; border-radius: 4px; background-size: cover; background-position: center; position: relative; overflow: hidden; border: 1px solid #222; transition: all 0.4s ease; }
...
data = [
    ...
    {"name": "GlobalFoundries", "state": "Germany", "region": "Global", "cap": "Mid", "lat": 51.12, "lon": 13.71, "sti": 88},
    ...
    # India Core
    {
        ...
        "rationale": "Engineered for 0.02% seismic vibration friction. Absolute flatness prevents multi-billion dollar EUV wafer spoilage."
    },
    {
        ...
        "rationale": "High topographical friction offset by strategic geographic defense and inexhaustible fresh water supply."
    },
    {
        "name": "Micron Sanand", "state": "Gujarat", "region": "India", "year": 2024, "cap": "Large", 
        "lat": 22.98, "lon": 72.37, "elev": 45, "terrain": "Industrial Plains",
        "w_name": "Narmada Canal", "w_lat": 23.10, "w_lon": 72.50, "w_dist": 20,
        ...
        "rationale": "Chosen for pre-existing grid stability and rapid modular scalability."
    },
    {
        "name": "CG Power-Renesas", "state": "Gujarat", "region": "India", "year": 2025, "cap": "Mid", 
        "lat": 23.00, "lon": 72.35, "elev": 43, "terrain": "Industrial Plains",
        "w_name": "Narmada Canal", "w_lat": 23.10, "w_lon": 72.50, "w_dist": 22,
        "m_name": "Regional Rail", "m_lat": 22.80, "m_lon": 72.20, "m_dist": 28,
        "l_name": "Ahmedabad Grid", "l_lat": 23.02, "l_lon": 72.57, "l_dist": 25,
        "bt": "Specialized OSAT for consumer power management ICs.", 
        ...
        "rationale": "Creates a localized high-density packaging cluster with shared logistics."
    },
    {
        "name": "TI Bangalore", "state": "Karnataka", "region": "India", "year": 1985, "cap": "Large", 
        "lat": 12.97, "lon": 77.59, "elev": 920, "terrain": "Deccan Plateau",
        "w_name": "Cauvery Basin", "w_lat": 12.40, "w_lon": 77.30, "w_dist": 85,
        "m_name": "HAL Data Transit", "m_lat": 12.95, "m_lon": 77.66, "m_dist": 12,
        "l_name": "Bangalore Grid", "l_lat": 12.97, "l_lon": 77.59, "l_dist": 2,
    ...
    },
    {
        ...
        "w_name": "Sutlej Tributaries", "w_lat": 30.90, "w_lon": 76.50, "w_dist": 28,
        "m_name": "Northern Rail", "m_lat": 30.50, "m_lon": 76.80, "m_dist": 24,
        "l_name": "Chandigarh Sec 17", "l_lat": 30.73, "l_lon": 76.77, "l_dist": 10,
        "bt": "ISRO space-grade & military radiation-hardened CMOS.", 
        ...
        "rationale": "Sacrificed maritime logistics to push sensitive military infrastructure deep inland."
    },
    {
        ...
        "m_name": "Hyderabad Cargo", "m_lat": 17.25, "m_lon": 78.43, "m_dist": 35,
        ...
        "rationale": "Chosen for pre-existing grid stability and rapid modularity."
    }
]
...
def render_dossier(n):
    ...
    with col2:
        # VULNERABILITY RADAR
        categories = ['Seismic', 'Water', 'Logistics', 'Geopolitical', 'Labor']
        fig = go.Figure()
...
with tab1:
    ...
    <div style='display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin-top: 10px; margin-bottom: 30px;'>
        ...
        <div class='hud-card'>
            <div class='hud-label'>Topographical Index</div>
            <div class='hud-value'>{avg_sti}</div>
        </div>
        <div class='hud-card'>
            <div class='hud-label'>Logistics Eff (LCP)</div>
    ...
    with col_map:
        st.markdown("<span style='font-family:Rajdhani; color:#d4af37; font-size:16px; font-weight:700;'>GEOSPATIAL THEATER</span> <span style='color:#666; font-size:12px;'>(CLICK PINS FOR DOSSIER)</span>", unsafe_allow_html=True)
        
        # Plotly Custom Map with "Glowing" Dual-Trace Pins
        fig_map = go.Figure()
        ...
        if not active_df.empty:
            ...
            fig_map.add_trace(go.Scattermapbox(
                ...
                mode='markers',
                marker=dict(size=24, color=active_df['color'], opacity=0.15),
                hoverinfo='none'
            ))
            ...
            fig_map.add_trace(go.Scattermapbox(
                ...
                marker=dict(size=8, color=active_df['color'], opacity=1.0, 
                            line=dict(width=1, color='#ffffff')),
                customdata=np.stack((active_df['name'], active_df['cap'], active_df['state']), axis=-1),
                hovertemplate="<b>%{customdata[0]}</b><br>%{customdata[2]} | %{customdata[1]} Cap<extra></extra>"
            ))

        # Render Map with click event listener
...
with tab2:
    ...
    fig_global.add_trace(go.Scattermapbox(
        ...
        customdata=np.stack((df['name'], df['cap']), axis=-1),
        hovertemplate="<b>%{customdata[0]}</b><br>%{customdata[1]} Cap<extra></extra>"
    ))
